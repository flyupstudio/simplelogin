...
//$PHPTHUMB_CONFIG['document_root'] = '/home/misha/Web/www/sporttur/core/lib/phpThumb/';
...
global $config;
$PHPTHUMB_CONFIG['document_root'] = $config['absolute-path'] ? rtrim($config['absolute-path'], '/') : (realpath((getenv('DOCUMENT_ROOT') && ereg('^'.preg_quote(realpath(getenv('DOCUMENT_ROOT'))), realpath(__FILE__))) ? getenv('DOCUMENT_ROOT') : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace(DIRECTORY_SEPARATOR, '/', dirname(__FILE__)))));
...
$PHPTHUMB_CONFIG['error_message_image_default'] = '/public/images/no_image.jpg';       // Set this to the name of a generic error image (e.g. '/images/error.png') that you want displayed in place of any error message that may occur. This setting is overridden by the 'err' parameter, which does the same thing.
